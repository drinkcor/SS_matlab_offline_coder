...
function [Result] = rt_snapVerification(axisIndex, StrategyType, FolderName)
...
%             fprintf('Time: %d ',currentIndex);
            ...
            while (Result.primiMarker+window_length <= localIndex)
                ...
                [hasNew_p,dAvg,dMax,dMin,dStart,dFinish,dGradient,dLabel,Result.primiDataFitPre,Result.primiWStart,Result.primiMarker] = rt_fitRegressionCurves(Wren_loc,StrategyType,FolderName,pType,Result.primiDataFitPre,Result.primiWStart,Result.primiMarker,axisIndex);    % fit window
                if(hasNew_p)
                    % Increase counter
        ...
        if(localIndex==globalIndex)
            ...
            if (Result.primiMarker_cl == Result.primiIndex )
                if (Result.primiLookForRepeat_cl) % if looking for repeat
                    ...
                    Result.primiIndex_cl = Result.primiIndex_cl+1;
                    Result.primiData_cl(Result.primiIndex_cl,:) = rt_MergePrimitives(startPrimitive,Result.primiData,nextPrimitive);
                else
                    Result.primiIndex_cl = Result.primiIndex_cl+1;
                    Result.primiData_cl(Result.primiIndex_cl,:) = Result.primiData(Result.primiMarker_cl,:);
                end
            end
            ...
            if (Result.MCMarker_cl == Result.MCIndex)
                if (Result.MCLookForRepeat_cl) % if looking for repeat
                    ...
                    Result.MCIndex_cl = Result.MCIndex_cl+1;
                    Result.MCData_cl(Result.MCIndex_cl,:) = rt_MergeCompositions(startPrimitive,Result.MCData,actionLbl,repeat_Lbl,nextPrimitive);
                else
                    Result.MCIndex_cl = Result.MCIndex_cl+1;
                    Result.MCData_cl(Result.MCIndex_cl,:) = Result.MCData(Result.MCMarker_cl,:);
                end
            end
            ...
            if (Result.llbehMarker_cl == Result.llbehIndex)
                if (Result.llbehLookForRepeat_cl) % if looking for repeat
                    ...
                    Result.llbehIndex_cl = Result.llbehIndex_cl+1;
                    Result.llbehData_cl(Result.llbehIndex_cl,:) = rt_MergeLowLevelBehaviors(startPrimitive,Result.llbehData,llbehLbl,repeat_Lbl,nextPrimitive);
                else
                    Result.llbehIndex_cl = Result.llbehIndex_cl+1;
                    Result.llbehData_cl(Result.llbehIndex_cl,:) = Result.llbehData(Result.llbehMarker_cl,:);
                end
            end
